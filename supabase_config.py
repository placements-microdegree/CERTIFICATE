...
TABLE_NAME = "certificate_users"  # Your Supabase table storing issued certificates

def get_certificate_by_id(certificate_id: str):
    """
    Fetch a certificate by ID from certificate_users.
    Returns a dictionary with student_name, course_name, completion_date, certificate_id, certificate_url.
    """
    certificate_id = certificate_id.strip().upper()
    print(f"[DEBUG] Looking for certificate_id: {certificate_id}")

    # Query certificate_users table
    user_cert_response = supabase.table(TABLE_NAME).select("*").eq("certificate_id", certificate_id).execute()
    print(f"[DEBUG] Query result: {user_cert_response.data}")

    if not user_cert_response.data or len(user_cert_response.data) == 0:
...
def add_sample_data():
    """
    Optional: Insert sample certificates for testing
    """
    if not supabase:
        print("⚠️ Supabase client not initialized")
        return
...
def get_certificate_by_id(certificate_id: str):
    """
    Fetch a certificate by ID from certificate_users.
    Case-insensitive match on certificate_id.
    """
    certificate_id = certificate_id.strip()
    print(f"[DEBUG] Looking for certificate_id (case-insensitive): {certificate_id}")

    try:
        # Query certificate_users table (case-insensitive)
        user_cert_response = (
            supabase.table(TABLE_NAME)
            .select("*")
            .ilike("certificate_id", certificate_id)   # 🔹 key change
            .execute()
        )
        print(f"[DEBUG] Query result: {user_cert_response.data}")
